...
## 3. Тестовый полигон: невыполненный план `tg-webhook-timeout`

Первая версия стенда была нечестной, и это выяснилось не сразу. Мы взяли три реальных плана из бэклога **Findrates**, уже реализованных и смёрженных, и прогнали ревьюеров на текущем `HEAD`. Модели набирали баллы за находки вида «эта фаза — no-op, всё уже сделано» — класс находок, которого в боевом контуре просто не бывает, потому что ревью плана всегда идёт **до** реализации. Ранжирование на этом стенде оказалось нерепрезентативным: Grok 4.5 набирал 10 находок, GLM-5.2 — 8 за 71 секунду, и оба выглядели сильнее, чем показали себя дальше.

Решающий стенд собрали заново на плане `tg-webhook-timeout` (бюджет и таймауты приёма Telegram-вебхука — лок на входящее сообщение, дедуп по `message-id`, инвариант «никаких тихих 200 ACK» для голосового и текстового пути) и на воркитри, отведённом к коммиту `0bcafc42^` — состоянию кода **до** реализации этого плана. Всем двенадцати моделям достался один и тот же промпт, один и тот же чекаут, один и тот же read-only доступ, включая символическую ссылку на `node_modules`, чтобы обе стороны видели внутренности `grammY` одинаково.

По результатам ручной проверки находок по коду выделились три класса дефектов, которые ловили разные модели:

1. **Пропущенный abort-сигнал вызывающей стороны:**
   Ветка `parse_new` (`parse-quote-request.ts:166`) не имеет таймаута со стороны вызывающего кода — в самом коде есть комментарий «callWithFallback has no caller signal». План поднимает границу ожидания до 50 секунд, но это превращает нынешний повторный ределивери в тихий 200 ACK — прямое нарушение собственного пункта плана «must-not 7». Находку независимо дали Claude Opus и Hunyuan 3.
2. **Слепая зона контрактного теста:**
   `telegram-webhook-contract.test.ts:46` импортирует изменяемый роут-хендлер напрямую, и поэтому не попадает в радиус поражения, заявленный планом, — то есть тест, который должен ловить регрессию, физически не может её поймать. Эту находку из двенадцати моделей дала только Hunyuan 3.
3. **Дедупликация после проверки лимита:**
   Дедуп по `message-id` стоит **после** проверки rate-limit, поэтому дубликат, пришедший уже после исчерпания лимита, всё равно успевает отправить клиенту второе видимое сообщение. Находку дал Codex, её же независимо подтвердил Hunyuan 3.

Отдельно вскрылся артефакт самого стенда: поиск `glob`/`grep` в `opencode` уважает `.gitignore`, поэтому `node_modules/` и `.claude/` не находятся поиском, хотя чтение по полному пути работает исправно. Из-за этого одна модель отказалась подтверждать вызовы `grammY` («проверить не могу — поиск ничего не находит»), а другая объявила несуществующим файл, который на самом деле существует в основном репозитории, но не в тестовом воркитри. Строку-противоядие про это ограничение добавили в промпт ревью.
...
Каждая модель получала одинаковый промпт, права на чтение репозитория и изолированное окружение на воркитри `0bcafc42^`. Оценивалось:
...
1. **Единственная находка, которую не дал больше никто:**
   Из двенадцати участников только Hunyuan 3 заметил, что контрактный тест `telegram-webhook-contract.test.ts:46` импортирует изменяемый роут-хендлер напрямую и поэтому не попадает в радиус поражения, который заявляет план, — то есть страховочный тест физически не может поймать регрессию, для отлова которой его писали. Ни Codex, ни Opus, ни один из платных участников этого не увидели.
